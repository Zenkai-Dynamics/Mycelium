...
- *Crash* (`NodeError`/`record_crash`): killing only Node B's `vllm
  ...
  repeatable, no artificial code paths involved.
...
6. **Scenario 2.** One lightweight stand-in registration (direct use of
   ...
   succeeds, filling the cap at 3. A second stand-in attempt is rejected;
   the exact `registration_rejected` reason is recorded.
7. **Scenario 3.** Node B's `vllm serve` subprocess is killed (its
   `mycelium-node` process stays up). Several client requests routed to
   Node B are recorded failing with a real error; `mycelium-coordinator-status`
   shows Node B's crash counter incremented. Node B's whole process is
   then `kill -9`'d for one real disconnect, and restarted (reconnects
   with its cached token, no re-auth). A larger batch of repeated client
   requests (enough to be statistically meaningful, not just 2-3) is sent;
   the split between Node A and Node B is recorded, confirming Node A is
   favored but Node B is still occasionally picked.
8. **Scenario 4.** `mycelium-coordinator-ban --identity <login>` is run
   against the real bound identity. Both currently-connected nodes (A and
   B) are confirmed disconnected immediately (`mycelium-coordinator-status`
   shows none registered). A further registration attempt from that
   identity (Node A restarting, or the stand-in script) is confirmed
   rejected with the banned-identity reason.
