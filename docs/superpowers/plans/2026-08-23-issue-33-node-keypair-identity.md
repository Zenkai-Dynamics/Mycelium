...
**Direct-`NodeRegistry` call sites** (bottom of the file — these bypass `_handle_registration` entirely and call `registry.register(...)` directly, so they don't need real cryptographic keys, just distinct arbitrary strings). Rewrite `test_complete_request_fails_over_to_healthy_node_when_first_pick_is_dead`, `test_complete_request_does_not_fail_over_on_timeout`, and `test_complete_request_returns_error_when_every_node_is_dead`'s `registry.register(...)` calls from `registry.register("node-a", "m", dead_ws)` to `registry.register("pubkey-a", "node-a", "m", dead_ws)` (and `"node-b"` → `registry.register("pubkey-b", "node-b", "m", healthy_ws)`, etc. — reorder every positional call to `(public_key, node_id, model, websocket)`). Update their `registry.list_nodes()` assertions to include `fingerprint`, e.g. `[{"node_id": "node-b", "model": "m", "fingerprint": crypto.fingerprint("pubkey-b")}]`, and `registry.get("node-b")` → `registry.get("pubkey-b")` inside `test_complete_request_fails_over_to_healthy_node_when_first_pick_is_dead`'s `reply_from_node_b` helper.
